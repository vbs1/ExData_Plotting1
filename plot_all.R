...
loadData <- function() {
  ...
  sqlQuery <- "select Date, Time, Global_active_power, Global_reactive_power, Voltage, Global_intensity, Sub_metering_1, Sub_metering_2, Sub_metering_3 from file where Date = '1/2/2007' or Date = '2/2/2007'"
...
#function to plot second plot
plot2 <- function(time, power, toFile) {
  if(toFile == TRUE) {
...
plot4 <- function(time, activepower, voltage, metering1, metering2, metering3, reactivepower, toFile) {
  if(toFile == TRUE) {
...
with(myData, {
  ...
  plot2(Date,Global_active_power, toFile = TRUE)
  plot3(Date,Sub_metering_1,Sub_metering_2,Sub_metering_3, toFile = TRUE)
  plot4(Date,Global_active_power,Voltage,Sub_metering_1,Sub_metering_2,Sub_metering_3,Global_reactive_power, toFile = TRUE)
})